...
class EtaController extends ApiController
{
    ...
    public function getAvailableUsers(){
        return User::select('id','name','surname','username')->orderBy('id','desc')->get();
    }
...
//        check that named user is available

//        $user = User::where('name', 'LIKE', "%{$request->username}%")->first();
        $user = User::where('id',$request->username)->first();

        if (!$user) {
            return $this->respondBadRequest('The user is not available in the system');
        }
        ...
        if ($app_for == 'taxi' || $app_for == 'delivery') {
            if ($request->has('vehicle_type')) {
                if ($request->is_outstation) {
                    $type = $zone_type->where('id', $request->input('vehicle_type'))->active()->whereHas('vehicleType', function ($query) {
                        $query->where('trip_dispatch_type', 'bidding');
                    })->first();
                } else {
                ...
                if ($request->is_outstation) {
                    $type = $zone_type->active()->whereHas('vehicleType', function ($query) {
                        $query->where('trip_dispatch_type', 'bidding');
                    })->first();
                } else {
            ...
            if ($request->has('vehicle_type')) {
                ...
                if ($request->has('transport_type')) {
                    if ($request->is_outstation) {
                        ...
                        })->where('id', $request->input('vehicle_type'))->active()->whereHas('vehicleType', function ($query) {
                            $query->where('trip_dispatch_type', 'bidding');
                        })->first();
                    } else {
                        $type = $zone_type->where(function ($query) use ($request) {
                    ...
                    if ($request->is_outstation) {
                        $type = $zone_type->where('id', $request->input('vehicle_type'))->whereHas('vehicleType', function ($query) {
                            $query->where('trip_dispatch_type', 'bidding');
                        })->active()->first();
                    } else {
                        $type = $zone_type->where('id', $request->input('vehicle_type'))->active()->first();
                ...
                if ($request->has('transport_type')) {
                    if ($request->is_outstation) {
                        ...
                        $type = $zone_type->where(function ($query) use ($request) {
                            $query->where('transport_type', $request->transport_type)->orWhere('transport_type', 'both');
                        })->active()->whereHas('vehicleType', function ($query) {
                            $query->where('trip_dispatch_type', 'bidding');
                        })->get();
                    } else {
