...
// Log::info($tax_percent);
    ...
    public function paymentConfirm(Request $request)
    {
        ...
        $request_detail = $driver->requestDetail()
            ->with(['userDetail', 'driverDetail'])
            ->where('id', $request->request_id)
            ->first();

        // Throw an exception if the user is not authorized for this request
        if (!$request_detail) {
            $this->throwAuthorizationException();
        }
        ...
        $data = [
            'user' => $user,
            'driver' => $driver,
            'invoiceNumber' => $request_detail->request_number,
            'rideDetails' => $request_detail,
        ];

//        NO NEEDED HERE
        /*
        // Generate PDF
        $pdf = PDF::loadView('new-invoice', $data);

        // Save the PDF temporarily
        $pdfPath = 'temp/invoices/' . uniqid() . '.pdf';
        ...
        try {
            ...
            Mail::to($user->email)->send(new PaymentConfirmationMail($data, Storage::path($pdfPath)));
        ...
        if ($user->userNdis) {
            try {
                Mail::to($user->userNdis->plan_manager_email)->send(new PaymentConfirmationMail($data, Storage::path($pdfPath)));
            } catch (\Exception $exception) {
                InvoiceMailLog::create([
                    'email' => $user->email,
                    'request_id' => $request->request_id,
        ...
        if ($user->userNiisq) {
            try {
                Mail::to($user->userNiisq->plan_manager_email)->send(new PaymentConfirmationMail($data, Storage::path($pdfPath)));
            } catch (\Exception $exception) {
                InvoiceMailLog::create([
                    'email' => $user->email,
                    'request_id' => $request->request_id,
        ...
        if ($user->userAgedCare) {
            try {
                Mail::to($user->userAgedCare->case_manager_email)->send(new PaymentConfirmationMail($data, Storage::path($pdfPath)));
            } catch (\Exception $exception) {
                InvoiceMailLog::create([
                    'email' => $user->email,
                    'request_id' => $request->request_id,
    ...
    public function paymentConfirmSpecial(Request $request)
    {
        $driver = auth()->user()->driver;
        ...
        $user = $request_detail->userDetail;
        $userAdvance = $request_detail->userDetail->userDetails;
        $driver = $request_detail->driverDetail;
        ...
        try {
            Mail::to("contact@freedomtransport.com.au")->send(new PaymentConfirmationMail($data, Storage::path($pdfPath)));
        } catch (\Exception $exception) {
            InvoiceMailLog::create([
                'email' => "contact@freedomtransport.com.au",
                'request_id' => $request->request_id,
                'error' => $exception->getMessage(),
                'type' => 'admin',
            ]);
...
//        DEVELOPER TEST
//        try {
//            Mail::to("greenavekariya.dds@gmail.com")->send(new PaymentConfirmationMail($data, Storage::path($pdfPath)));
//        } catch (\Exception $exception) {
//            InvoiceMailLog::create([
//                'email' => "greenavekariya.dds@gmail.com",
//                'request_id' => $request->request_id,
//                'error' => $exception->getMessage(),
//                'type' => 'dev',
//            ]);
//        }
        ...
        if ($user->userNdis && $user->userNdis->participant) {
            try {
                Mail::to($user->userNdis->plan_manager_email)->send(new PaymentConfirmationMail($data, Storage::path($pdfPath)));
            } catch (\Exception $exception) {
                InvoiceMailLog::create([
        ...
        if ($user->userNiisq && $user->userNiisq->participant) {
            try {
                Mail::to($user->userNiisq->plan_manager_email)->send(new PaymentConfirmationMail($data, Storage::path($pdfPath)));
            } catch (\Exception $exception) {
                InvoiceMailLog::create([
        ...
        if ($user->userAgedCare && $user->userAgedCare->participant) {
            try {
                Mail::to($user->userAgedCare->case_manager_email)->send(new PaymentConfirmationMail($data, Storage::path($pdfPath)));
            } catch (\Exception $exception) {
                InvoiceMailLog::create([
