...
class InstantRideController extends BaseController
{
    ...
    */
    public function createRequest(CreateTripRequest $request)
    {
        ...
        if (!$user) {
            return $this->respondBadRequest('The user is not available in the system .'.$request->name);
        }
        ...
        $ad_hoc_user_params['name'] = $user->name;
        $ad_hoc_user_params['mobile'] = $request->mobile ?? null;
