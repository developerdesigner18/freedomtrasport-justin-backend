...
//        INSERTING USER
        ...
        if ($request->has('user_aged_details') && !empty($request->user_aged_details)) {
            $user->userAgedCare()->create($request->user_aged_details);
        }

        if ($request->has('user_ndis_details') && !empty($request->user_ndis_details)) {
            $user->userNdis()->create($request->user_ndis_details);
        }

        if ($request->has('user_niisq_details') && !empty($request->user_niisq_details)) {
            $user->userNiisq()->create($request->user_niisq_details);
        }

        if ($request->has('user_private_details') && !empty($request->user_private_details)) {
            $user->userPrivate()->create($request->user_private_details);
        }
